...
状态：`ACCEPTED FOR P0 / P6 HARDENING`；日期：2026-09-06；对应：P0-07、P0-10、U-004。
...
- `script/p0.sh skill`（closure HEAD `ce4a71c`，10/10 通过，使用 SHA-256 校验过的 Wasmtime 48.0.1 库）：合法 guest 正常执行；deadline 终止无限循环 guest；取消标志及时结束运行；导入、模块、响应上限和路径策略均按合同拒绝。
- `script/quality.sh` 的 XPC 端到端 fixture：宿主 app 连接内嵌 XPC service 执行真实 guest，容器 temp 报告 `{"status":"success"}`，非 success 令质量门失败。
- 早期 P0 quality 产物已记录 capability callback broker、XPC fixture 与 crash/reconnect 演练；closure 的两次 quality 运行在 XCUITest accessibility 阶段停止，因此不把该次运行写成 exit 0。远端 CI、真实能力执行和恶意矩阵留给 P6/P9。
...
若产品隔离矩阵失败，保留请求/结果合同和策略测试，暂停可执行 Skill 交付，不退回无隔离执行。U-004 的信任根、撤销、真实能力执行和恶意矩阵仍未关闭，由 P6 收口。
