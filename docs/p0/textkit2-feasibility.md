...
Status: `CHANGES_REQUIRED` for the full exit gate; the compile-time probe and Unicode range fixture pass, while real IME, attachment, undo/redo, VoiceOver and latency measurements remain `NOT_RUN`.
...
- The range fixture is exercised when the probe host builds; an automated XCTest for real marked-text input is intentionally not claimed because it would not prove an actual third-party IME session.
