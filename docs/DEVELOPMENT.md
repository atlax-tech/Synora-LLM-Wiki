...
- 影响 AI/外部集成/Skill 时包含超时、重试、幂等、审计和故障注入。
- 文档、ADR、权限说明和 traceability matrix 同步更新。
- 没有引入密钥、用户正文或敏感提示词日志。

### 开发准则

1. **本地提交先行**：用户操作先成为本地原子事务；索引和 AI 只能后置。
2. **主线程只做 UI**：解析、缩略图、索引、模型、外部集成和 Skill 全部有可取消后台边界。
3. **协议隔离**：Feature 不直接依赖 GRDB、供应商 SDK、WASI 或 Keychain。
4. **稳定标识**：Record/Block/Asset/Operation 在创建时即获得稳定 ID。
5. **写入统一**：用户、AI、导入都通过 Command → ChangeSet → Transaction 路径。
...
- 领域/存储/集成/AI 安全模块至少两类证据：自动测试 + 设计/架构人工审查。
...
|---|---|---|---|---|---|
| `FR-…` | `P…` | `P…-…` | `TC-…` | 日志/截图/报告链接 | Planned/In Progress/Passed/Blocked/Deferred（不计入适用门槛） |
...
- 准备本阶段适用的基准数据、Mac 设备、供应商配置与免费可用权限。Apple 付费账号、Team、云容器、MusicKit/WeatherKit entitlement、Developer Token 与公证凭据不是启动条件；若某增强能力确认依赖付费会员，仅将该能力按 PRODUCT §5.3 暂缓，继续独立本地任务。
